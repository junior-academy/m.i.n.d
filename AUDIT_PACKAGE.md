...
The full audit package is kept under ignored `outputs/v2/` directories. It contains raw per-run artifacts used to verify the paper claims, but it does not need to be included with the paper artifact package.
...
Together with the paper artifact package, these directories preserve the audit trail for the local IIIa, MOABB, uncertainty-score, calibration, and controller-replay claims.
