...
persist beyond BCI IV 2a, although ECE worsens in that tiny subset. EEGNet is
included as an optional modern deep-learning comparison, not as the project
focus. The central contribution is not a claim of state-of-the-art accuracy; it
is a reproducible framework for evaluating whether a BCI decoder is reliable
enough to act.
\end{abstract}
...
M.I.N.D. is built around a human-centered control principle:
\[
...
not use EEGNet inside the ensemble and does not tune the architecture to chase
state-of-the-art performance. The optional implementation follows the standard
...
M.I.N.D. reframes motor-imagery EEG decoding as a reliability problem rather
than a pure accuracy problem. On BCI Competition IV 2a, the calibrated LDA+SVM
ensemble improves accuracy, ECE, and Brier score over calibrated LDA. MOABB
smoke validation suggests that the accuracy and Brier-score gains can persist
outside the primary benchmark, while also revealing a calibration limitation:
the ensemble may need post-vote recalibration to improve ECE consistently.

The strongest claim is therefore measured and defensible: a lightweight,
interpretable, calibrated ensemble with selective prediction can make BCI
commands more reliable, and the project now evaluates that claim with the
right reliability metrics across both the primary benchmark and external
MOABB datasets. EEGNet provides a modern comparison point, but the novelty of
M.I.N.D. is the reliability-gated assistive-control framing and the
